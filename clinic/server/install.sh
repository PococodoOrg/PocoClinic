...
if [[ ! -f "${ENV_FILE}" ]]; then
  ...
  chmod 640 "${ENV_FILE}"
  echo "Created ${ENV_FILE} — edit JWT_*, DOCUMENT_ENCRYPTION_KEY, ALLOWED_ORIGIN before starting."
else
...
echo ""
echo "Next steps (beginner guide: devices/raspberry-pi/setup.md):"
echo "  1. Edit ${ENV_FILE} — replace REPLACE-ME secrets; set ALLOWED_ORIGIN to your browser URL"
echo "     Generate: openssl rand -base64 48  (twice) and openssl rand -base64 32"
echo "     First HTTP bring-up: COOKIE_SECURE=false (remove after TLS — see tls-lan.md)"
echo "  2. sudo ${INSTALL_ROOT}/bin/migrate"
echo "  3. sudo systemctl enable --now pococlinic pococlinic-ops-helper"
echo "  4. Open http://<pi-ip>:8080/login/admin — credentials in:"
echo "     sudo cat /var/lib/pococlinic/bootstrap-admin-once.txt"
echo "  5. Before patients: LAN TLS + remove COOKIE_SECURE=false"
echo "     See devices/raspberry-pi/tls-lan.md and hardening.md"
