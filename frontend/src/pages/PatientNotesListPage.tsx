...
export default function PatientNotesListPage() {
  ...
  if (error || (!isLoading && !patient)) {
    return <PatientChartLoadError patientId={id} message="Could not load patient notes." />;
  }

  return (
    <Stack gap="lg">
      {patient && <PatientBriefHeader patient={patient} sectionTitle="Clinical notes" />}
      <Paper radius="md" p="xl" withBorder pos="relative">
        <LoadingOverlay visible={isLoading} />
        {patient && <PatientNotesSection patientId={patient.id} />}
      </Paper>
    </Stack>
  );
}
